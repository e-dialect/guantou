...
当前进入 Entry / Recording V2 兼容切换期。新主路径以 `Entry` 表示同一个词及其读音身份，
以 `EntrySense` 保存相关分义，以 `Concept` 连接 WALK、RUN 等跨词条概念；`Recording`
独立保存音频及贡献者原始大意，并通过 `primary / mention / competing` 多对多关联词条。
同形的“行走的行”和“银行的行”会返回为不同 Entry，Concept 关联也不会自动合并词条。

`GET /entries/` 支持写法、来源、状态、IPA、罗马字、概念、是否有录音和地区范围筛选。
地区默认 `exact`；只有显式使用 `dialect_scope=subtree` 才检索所选节点及其后代，上层
地区的宽泛贡献不会被反向推断为每个下级地区都使用。`POST /recordings/` 最低要求音频、
使用地区和大意；没有现成词条时服务端建立一条无专业写法也合法的初稿 Entry，并保存
不可覆写的用户原话证据。旧接口在客户端完成切换前继续可用。
...
客户端能力由构建能力和 `GET /site-settings/capabilities` 的远程开关取交集，远程开关只能关闭、不能凭空启用未编译能力。`POST /product-events/` 只接受固定事件和低基数枚举字段；搜索原文、内容 ID、账号资料、方言限定码、录音内容及位置字段会被拒绝。原始事件最多保留 90 天，跨期只保留日汇总。完整口径见 [`PLATFORM_CAPABILITIES_ANALYTICS.md`](../../PLATFORM_CAPABILITIES_ANALYTICS.md)。
